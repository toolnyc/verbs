...
describe('isEventPast', () => {
  ...
  // --- Basic: event with no time_end uses 24h buffer ---
  ...
  it('event that started 12 hours ago (no time_end) is not past', () => {
    setNow('2026-02-08T08:00:00Z');
    expect(isEventPast({ date: '2026-02-07T20:00:00Z' })).toBe(false);
  });

  it('event that started 25 hours ago (no time_end) IS past', () => {
    setNow('2026-02-08T21:00:00Z');
    expect(isEventPast({ date: '2026-02-07T20:00:00Z' })).toBe(true);
  });

  it('event at exactly 24h after start (no time_end) is not past', () => {
    setNow('2026-02-08T20:00:00Z');
    expect(isEventPast({ date: '2026-02-07T20:00:00Z' })).toBe(false);
  });
  ...
  it('time_end BEFORE date falls back to 24h buffer', () => {
    // Event starts at 8pm, time_end is 2am same day (before start = bad data)
    // Should fall back to 24h buffer, not treat as already past
  ...
  it('time_end equal to date falls back to 24h buffer', () => {
  ...
  it('null time_end uses 24h buffer', () => {
    setNow('2026-02-07T21:00:00Z');
    expect(isEventPast({
      date: '2026-02-07T20:00:00Z',
      time_end: null,
  ...
  it('undefined time_end uses 24h buffer', () => {
    setNow('2026-02-07T21:00:00Z');
    expect(isEventPast({
      date: '2026-02-07T20:00:00Z',
      time_end: undefined,
